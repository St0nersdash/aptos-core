...
impl TransactionOptions {
    ...
    pub async fn submit_transaction(
        ...
        if now_usecs > state.timestamp_usecs + ACCEPTED_CLOCK_SKEW_US {
            return Err(CliError::UnexpectedError(format!("Failed to send transaction, clock is more than {} seconds ahead of the blockchain {}", ACCEPTED_CLOCK_SKEW_US, state.timestamp_usecs / US_IN_SECS )));
        } else if now_usecs < state.timestamp_usecs - ACCEPTED_CLOCK_SKEW_US {
            return Err(CliError::UnexpectedError(format!("Failed to send transaction, clock is more than {} seconds behind of the blockchain {}", ACCEPTED_CLOCK_SKEW_US, state.timestamp_usecs / US_IN_SECS )));
        }
