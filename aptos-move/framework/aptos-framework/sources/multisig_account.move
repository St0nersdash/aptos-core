...
module aptos_framework::multisig_account {
    ...
    /// account.
    const SALT: vector<u8> = b"aptos_framework::multisig_account";
    ...
    #[view]
    public fun last_resolve_transaction_id(multisig_account: address): u64 acquires MultisigAccount {
        let multisig_account_resource = borrow_global_mut<MultisigAccount>(multisig_account);
    ...
    public entry fun test_remove_owners_with_fewer_remaining_than_signature_threshold_should_fail(
        ...
        let owner_2_addr = address_of(owner_2);
        let owner_3_addr = address_of(owner_3);
        create_with_owners(owner_1, vector[owner_2_addr, owner_3_addr], 2, vector[]);
        let multisig_account = get_possible_multisig_account_address(owner_1_addr, vector[]);
        let multisig_signer = &create_signer_for_test(multisig_account);
