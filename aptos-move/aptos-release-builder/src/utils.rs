...
pub(crate) fn generate_next_execution_hash_blob(
    ...
    if next_execution_hash == "vector::empty<u8>()".as_bytes() {
        ...
        emitln!(writer, "x\"{}\",", hex::encode(next_execution_hash));
        writer.unindent();
        emitln!(writer, ");");
    }
}
//
// pub(crate) fn generate_next_execution_hash_string(
//     writer: &CodeWriter,
//     next_execution_hash: Vec<u8>,
// ) -> String {
//     // println!("Next execution hash string in framework {:?}", hex::encode(next_execution_hash.clone()));
//     for (i, b) in next_execution_hash.iter().enumerate() {
//         emit!(writer, "{}u8,", b);
//     }
